/**
 * Wire shape for a showcase chat message — what both the REST history endpoint
...
public record ShowcaseChatMessageDTO(
        String id,
        String authorUserId,
        String authorName,
     ...
     */
    public static ShowcaseChatMessageDTO redactedFor(ShowcaseChatMessage row, boolean isHost) {
        boolean hideBody = row.isModerated() && !isHost;
        return new ShowcaseChatMessageDTO(
                row.getId(),
                row.getAuthorUserId(),
                row.getAuthorName(),
