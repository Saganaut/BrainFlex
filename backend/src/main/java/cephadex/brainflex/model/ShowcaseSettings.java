...
public class ShowcaseSettings {
    ...
    // is layered on top of this flag: reactions only fire when the showcase
    // flag is on AND the current element opts in.
    private boolean reactionsEnabled = true;
    private boolean chatEnabled = true;

    // Team mode (chunk 12). When `teamMode` is true, createShowcase seeds
    ...
    // for the rest of the show, deterministic on showcase id so reconnects
    // see the same order).
