...
public sealed interface DeckElement
        ...
        /**
         * The shared chrome (display / response / media / audit fields). Never null in
         * practice.
         */
        ...
        default Map<String, Object> styledTitle() {
                return chrome().styledTitle();
        }

        /** Auto-advance after this many seconds. 0 = host advances manually. */
        default int displaySeconds() {
                return chrome().displaySeconds();
        }

        /**
         * Private notes shown only to the host during play. Never broadcast to
         * participants.
         */
        default String speakerNotes() {
                return chrome().speakerNotes();
        }
        ...
        default String videoUrl() {
                return chrome().videoUrl();
        }

        default String audioUrl() {
                return chrome().audioUrl();
        }

        /**
         * Optional MediaAsset id (kind=VIDEO_FILE or VIDEO_EMBED). Wins over
         * {@link #videoUrl()} when set.
         */
        default String videoAssetId() {
                return chrome().videoAssetId();
        }
        ...
        default String audioAssetId() {
                return chrome().audioAssetId();
        }

        default MediaPosition mediaPosition() {
                return chrome().mediaPosition();
        }
         ...
         */
        default boolean scored() {
                return chrome().scored();
        }
         ...
         */
        default boolean survey() {
                return chrome().survey();
        }
         ...
         */
        default Integer multipleSelections() {
                return chrome().multipleSelections();
        }

        /**
         * Host-controlled freeze toggle. NOT_ACCEPTING_RESPONSES rejects submissions.
         */
        default ResponseMode responseMode() {
                return chrome().responseMode();
        }
         ...
         */
        default int bestAnswerPoints() {
                return chrome().bestAnswerPoints();
        }

        /** Strategy used to award points during the VOTE phase. */
        default BestAnswerScoring bestAnswerScoring() {
                return chrome().bestAnswerScoring();
        }

        /**
         * User who first authored this element. Set by the backend on add; null for
         * system seeds.
         */
        default String createdByUserId() {
                return chrome().createdByUserId();
        }

        /**
         * User who most recently edited this element. Stamped server-side on every
         * update.
         */
        default String lastEditedByUserId() {
                return chrome().lastEditedByUserId();
        }

        /** Timestamp the element was first added to the deck. Stamped server-side. */
        default Instant createdAt() {
                return chrome().createdAt();
        }

        /** Timestamp of the most recent edit. Stamped server-side on every update. */
        default Instant updatedAt() {
                return chrome().updatedAt();
        }

        /**
         * Per-element tag references (independent of the deck's tagIds). Empty by
         * default.
         */
        default List<String> tagIds() {
                return chrome().tagIds();
        }

        /** Caption shown beneath the media slot — author-controlled. */
        default String mediaCaption() {
                return chrome().mediaCaption();
        }

        /** Accessibility text for the media slot. */
        default String altText() {
                return chrome().altText();
        }

        /** When false, audience emoji reactions are suppressed for this element. */
        default boolean reactionsEnabled() {
                return chrome().reactionsEnabled();
        }
