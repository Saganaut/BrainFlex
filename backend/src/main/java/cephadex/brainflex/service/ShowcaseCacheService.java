/**
 * Redis cache for active Showcase state.
 * Stores each session as JSON keyed by "game:{roomCode}" with a 2-hour TTL.
 * During an active game, ShowcaseService reads/writes here instead of MongoDB on every
 ...
 * ShowcaseService falls back to MongoDB reads automatically when the cache misses.
 */
...
public class ShowcaseCacheService {
    ...
    private static final Logger log = LoggerFactory.getLogger(ShowcaseCacheService.class);
    private static final String KEY_PREFIX = "showcase:";
    private static final Duration TTL = Duration.ofHours(2);

    private final StringRedisTemplate redis;
    private final ObjectMapper objectMapper;

    public ShowcaseCacheService(StringRedisTemplate redis, @Qualifier("redisObjectMapper") ObjectMapper objectMapper) {
        this.redis = redis;
        this.objectMapper = objectMapper;
    }

    /** Serialize and store the session; refreshes the TTL on every write. */
    public void put(Showcase session) {
        try {
    ...
    public Optional<Showcase> get(String roomCode) {
        try {
            String json = redis.opsForValue().get(KEY_PREFIX + roomCode);
            if (json == null) return Optional.empty();
            return Optional.of(objectMapper.readValue(json, Showcase.class));
        } catch (Exception e) {
