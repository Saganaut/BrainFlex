package cephadex.brainflex.web;
 ...
 *
 * WHY THIS EXISTS: the frontend stamps each API call with an {@code X-Request-Id}
 * header. This filter adopts that id (or mints one when absent), exposes it back
 ...
 * {@link GlobalExceptionHandler} echoes the {@code traceId} in error bodies — so
 ...
 * AuthorizationFilter.class)} so the authenticated principal is already resolved
