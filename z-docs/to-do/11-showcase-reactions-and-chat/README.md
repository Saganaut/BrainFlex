...
**Status:** Backend complete (2026-05-19). Models, repos, settings flags, STOMP destinations, REST fallbacks, Redis-backed reaction aggregation, rate limiter, emoji allow-list, host moderation, and `ShowcaseService` accept methods landed with tests (244/244 backend tests pass). Frontend codegen run — `useSendReactionMutation` / `useSendChatMutation` / `useListChatQuery` / `useModerateChatMutation` are wired. **Player + host UI (ReactionBar / ReactionRain / ChatPanel) is deferred to chunk 13**, following the same pattern as the chunk 07/08/09 player surfaces — building one engagement UI in isolation would establish a player-surface pattern the other deferred work would have to match.
...
- [x] `Reaction` + `ShowcaseChatMessage` models + repos + indexes — `model/Reaction.java`, `model/ShowcaseChatMessage.java`, `repository/ReactionRepository.java`, `repository/ShowcaseChatMessageRepository.java` with compound `(showcaseId, sentAt DESC)` indexes on each
- [x] STOMP destinations registered in `ShowcaseWebSocketController` — `/app/showcase/{roomCode}/reaction`, `/app/showcase/{roomCode}/chat`, `/app/showcase/{roomCode}/chat/moderate` broadcasting on `/topic/showcase/{roomCode}/reaction` and `/topic/showcase/{roomCode}/chat`
- [x] `ShowcaseService` accept methods + rate limiting + emoji validation — `acceptReaction`, `acceptChat`, `moderateChatMessage`, `listChatHistory`, gated by `ShowcaseSettings.reactionsEnabled/chatEnabled` and per-element `DeckElement.reactionsEnabled()` from chunk 10
- [x] REST fallback endpoints + tests — `ShowcaseController` exposes `POST /reactions`, `POST /chat`, `GET /chat?page=&size=`, `PUT /chat/{messageId}/moderate`
- [x] Redis-backed aggregation for reaction counts — `ShowcaseCacheService.incrementReactionCount` / `getReactionCounts` (HINCRBY on `reactions:counts:{roomCode}:{elementId}`)
- [x] `ShowcaseSettings.reactionsEnabled`, `chatEnabled` flags (coordinate with chunk 13) — defaults `true`; wired through `CreateShowcaseRequest`
- [x] `EmojiAllowList` static allow-list (13 default emojis) + `ShowcaseRateLimiter` (Redis ZSET sliding window: reactions 30/min, chat 20/min) — both fail-open on Redis outage so engagement degrades gracefully
- [ ] `ReactionBar` (player) + `ReactionRain` (host) components — *deferred to chunk 13 (player-surface pass)*
- [ ] `ChatPanel` (host + player) component with host moderation — *deferred to chunk 13*
- [ ] Optimistic chat send + STOMP reconcile — *deferred to chunk 13*
...
- [x] Backend tests pass — 244/244 including 8 new `ShowcaseServiceTest` cases for reactions (persist + broadcast, emoji rejection, settings gate, rate-limit) and chat (host flag, empty/oversize rejection, settings gate, moderation flip, non-host moderation forbidden)
