...
**Status:** Hooks + all component migrations landed; per-item card extraction and drag-to-reorder remain follow-up work.
**Depends on:** nothing
**Unblocks:** future slide-content refactors (drag-to-reorder for non-MCQ collections, per-item card components, shared structural patterns)
...
- [ ] Drag-to-reorder for non-MCQ collections (mirror MCQ `handleOptionDragEnd` / `moveMcqOption` endpoint). Will need backend endpoints for `moveAllocationOption`, `moveMatchingPair`, `moveRankingItem`, `moveScaleStatement` to avoid full-element rewrites.
...
- [ ] Split block-kind editors into their own components inside `SlideContent/` subfolder and back them with per-block hooks composed on top of `useSlideEditor`.
