...
### 2c. Player chrome stats (`currentStreak`, `longestStreak`, `accuracy`, `reactionsSent`) duplicated between `InteractiveSessionPlayer` and `PlayerPlacement` — **RESOLVED**

Extracted to `cephadex.brainflex.model.PlayerEndStats(int longestStreak, double accuracy, int reactionsSent)` and embedded as a single `endStats` field on `InteractiveSessionPlayer`, `PlayerPlacement`, and `GameHistoryEntry`. The fields that are intentionally not part of the bundle stay top-level on their owning class: `InteractiveSessionPlayer.currentStreak` (live-only), `InteractiveSessionPlayer.speedBonusTotal` and `PlayerPlacement.speedBonusTotal` (scoring-derived), and `GameHistoryEntry.currentStreakAtEnd` (history-only).

Wire and persistence shape now nests these three values under `endStats`. See [migrations-needed.md](migrations-needed.md#playerendstats-extraction--2c-of-java-model-issuesmd) for the Mongo migration script and frontend follow-ups.
...
### DEFERRED ### 2h. `Membership.monthlyInteractiveSessionCount` lives on `User.membership` but is gameplay state, not billing state
...
### DEFERRED ### 2i. `User` ↔ `Organization`: ownership/seat fields are split
...
### DEFERRED ### 3b. `RoundResultMessage` nested records could be top-level
...
5. ~~**Flatten the four "container class with nested records" DTOs (#3a).**~~ ✅ Done — `GalleryImageDTO`, `MediaAssetDTO`, `OrganizationDTO`, `ThemeDTO` are now top-level records. `TagDTO` is still a container — keep on the back-burner if codegen needs cleaning up.
...
8. ~~**Extract a `BillingState` value type shared by `Membership` and `OrganizationPlan` (#3c)**~~ ✅ Done — see 3c above. Still pending: move usage counters off `Membership` (#2h). Improves the billing surface ahead of the Stripe integration.
