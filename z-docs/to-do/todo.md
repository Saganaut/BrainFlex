...
### Deck editor (CreateDashboard)

All four items below are now scoped under [chunk 23 — Deck editor shell polish](./23-deck-editor-shell-polish/README.md):

- [ ] Navbar: wire the existing Preview and Start (showcase) buttons (currently `console.log` stubs in `CreateDashboard.tsx`)
...
| Item                              | Status                                                            |
| --------------------------------- | ----------------------------------------------------------------- |
| Live results display              | Covered by [chunk 13](./13-showcase-settings-and-player-additions/README.md); `RoundResult.tsx` / `ScoreBoard.tsx` exist for post-round |
| Player status indicators          | Covered by [chunk 13](./13-showcase-settings-and-player-additions/README.md) (`disconnected`, `lastSeenAt` fields) |
...
| Chat / communication panel        | Covered by [chunk 11](./11-showcase-reactions-and-chat/README.md) |
| Control panel (host)              | Host view items in [chunk 13](./13-showcase-settings-and-player-additions/README.md) |
| Connected player info             | Covered by [chunk 13](./13-showcase-settings-and-player-additions/README.md) |
| Connection status                 | Partial — `WsErrorBanner.tsx` covers WS errors; per-player status surface lands with [chunk 13](./13-showcase-settings-and-player-additions/README.md) |
...
| Per-kind player surfaces          | Deferred to [chunk 13](./13-showcase-settings-and-player-additions/README.md) per the README "Deferred work" section — Drawing canvas, Word Cloud / Allocation / Matching player views, per-kind `PlayPage` dispatch |
| Leaderboard (account)             | `components/Leaderboard/Leaderboard.tsx` exists; per-game leaderboard polish lands with [chunk 13](./13-showcase-settings-and-player-additions/README.md) |
| Feedback / response indicators    | `AnswerOptions.tsx` already styles correct/wrong; a unified feedback component would land with [chunk 13](./13-showcase-settings-and-player-additions/README.md) |
